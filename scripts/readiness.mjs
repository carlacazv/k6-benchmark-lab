...
if (!fs.existsSync(planFile)) {
  console.error(`Performance plan not found: ${planFile}`);
  process.exit(2);
}

let plan;
try {
  plan = parseSimpleYaml(fs.readFileSync(planFile, 'utf8'));
} catch (error) {
  console.error(`Invalid performance plan: ${error.message}`);
  process.exit(2);
}

const assessment = assessPlan(plan, scenario);
const env = runtimeEnv(plan, assessment);
fs.mkdirSync(outDir, { recursive: true });
fs.writeFileSync(path.join(outDir, 'readiness.json'), JSON.stringify({ source: planFile, assessment, runtimeEnv: env }, null, 2));
fs.writeFileSync(path.join(outDir, 'readiness-report.md'), renderMarkdown(plan, assessment, planFile));
